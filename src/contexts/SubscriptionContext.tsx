...
export const SubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Effect to handle session updates from AuthContext
  useEffect(() => {
    // This will be called when the provider is mounted
  }, []);
...
// Create a global ref to track the setSession function
...
// Update the provider to expose setSession globally
export const SubscriptionProviderWithGlobalSession: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, dispatch] = useReducer(subscriptionReducer, initialState);
  const sessionRef = useRef<Session | null>(null);
  const debounceTimer = useRef<NodeJS.Timeout | null>(null);
  const isRequestInFlight = useRef(false);
